...
zcanada="No"
zyear="45" ;this changes the year prefix for all but comments at the beginning of a new fiscal year and ship lookup files (for ogs items),
...
;openfile "MailManifest" ;removed by Ryan 1-6-22 cuz he barely uses the thing
;if info("Records") >1
...
vmem=1
 ...
 //stop ; for Facilitation_Avoid_Finder safety process
...
call ".salestax"
 	...
 	endcase
 	vmemflag=0
	end:˛/.retotalˇ˛.salestaxˇlocal taxstates, noship, taxitems, vgroupvermonttax, vMDMAtaxable, vArraySize, vPosIncrement, vCheckArray
...
If «BalDue/Refund» ≥ zcheckmin OR (zcanada="Yes" AND «BalDue/Refund» > .10) OR vmail="pBm"
	...
	YesNo "Do you want to write a check for "+pattern(«BalDue/Refund», "$#.##")+"?"
   	...
   	YesNo "Do you want to refund the credit card for "+pattern(«BalDue/Refund», "$#.##")+"?" //comment this line out when doing batch recall refunds, usually with a macro like ".3759 refund"
  	if clipboard()="Yes"  //comment this line out when doing batch recall refunds, usually with a macro like ".3759 refund"
    	zmethod="Credit Card"
   	endif  //comment this line out when doing batch recall refunds, usually with a macro like ".3759 refund"
   	endif
...
˛/.orderchangerˇ˛--ˇ˛/--ˇ˛(doodah)ˇ˛/(doodah)ˇ˛ImportOrders/7ˇglobal checkprocedure, discprocedure, zmessage
	if folderpath(dbinfo("folder","")) CONTAINS "tally.seeds"
	if info("unixusername") CONTAINS "CampanulaTranchelium" or info("unixusername") CONTAINS "fedco"
...
set var1 to "«file»"
    set name of document file var1 of folder "zoffice files" of folder "45tally.seeds" of folder "Desktop" of folder "fedco" of folder "Users" of startup disk to var1 & "√"
end tell}
;closefile
...
//stop ;used when Facilitation runs find avoid procedure
...
field OrderNo
sortup
;zmemberwindow="10.14:Users:CampanulaTranchelium:Desktop:"+zyear+"tally.seeds:member_list"
zmemberwindow="Depurple:Users:fedco:Desktop:"+zyear+"tally.seeds:member_list"
...
field OrderNo
sortup
zdiscountwindow="10.14:Users:CampanulaTranchelium:Desktop:"+zyear+"tally.seeds:seeds all"
zdiscountwindow="Depurple:Users:fedco:Desktop:"+zyear+"tally.seeds:seeds all"
	if info("files") notcontains "seeds all"
...
;this only works if you have the up-to-date Seeds All file in your Tally folder. Up to date Sees All lives in fedco˛/OfficeInfo/øˇ˛show pullers&checkersˇopenfile "pullers&checkers"
...
until  clipboard()="No"
˛/patronageUpdate/9ˇ˛huh/8ˇ;;this works on a selection
//testing to turn all 2744 in backorders. this is to see if recall, which is a refund for
//those who do not accept subs and a replace with sub for those who do, can be done by reverting to b/o and then doing backorder process
//RMM 4-6-22 SEEMS TO WORK
...
loop
linenu=arraysearch(Order,"*2744*", linenu,¶)
stoploopif linenu=0
until info("found")

Order=arraychange(Order,arraychange(extract(Order,¶,linenu),"b",5,¬),linenu,¶)
call reworkchanges/®

˛/huh/8ˇ˛huh2ˇ;this copies a list of all the fields to the clipboard

;clipboard()=dbinfo("fields","")
;message clipboard()

;this copies the unixusername to the clipboard


clipboard()=info("unixusername")
message clipboard()
...
˛/huh2ˇ˛huh3ˇ//this is ReprintPicksheet with some adjustments to be able to print selected orders
waswindow= info("WindowName")
if PickSheet=""
//if PickSheet≠""
zcanada="No"
...
;else
;message "This order has never had a picksheet printed."
endif˛/huh3ˇ˛yupˇlocal vCount,vRecords
...
message vCount˛/yupˇ˛SeedOriginˇ;has not been used in years
stop
each=""
origin=""
linenu=1
...
str(StateTax)+¬+str(CountyTax)+¬+str(CityTax)+¬+str(SpecialTax)+¬+str(OrderTotal)
 ...
 openfile "+@raya"
;call "monthlytotals/t"˛/seedsmonthlyˇ˛charactersˇmessage length(Order)˛/charactersˇ˛.246 refundˇ//this works on a selection
...
loop
linenu=arraysearch(Order,"* 246*", linenu,¶)
stoploopif linenu=0
until info("found")
...
//comment this section out if order still has Backorders ;Backorder is empty
// leave these lines if refund will be issued. comment out lines in .refund if CC batch refunds will happen to avoid having to hit "enter" after every refund
if str(OrderNo) notcontains "."
«BalDue/Refund»=Paid-GrTotal
call ".refund"
Paid=Paid-«BalDue/Refund»
...
until info("stopped")
˛/.246 refundˇ˛.3189 refundˇ//this works on a selection
...
loop
linenu=arraysearch(Order,"*3189*", linenu,¶)
stoploopif linenu=0
until info("found")
...
until info("stopped")
˛/.3189 refundˇ˛.2744 refundˇ//this works on a selection
...
until info("stopped")
˛/.2744 refundˇ˛.1648 refundˇstop
local delicataline
loop
delicataline=""
linenu=1
loop
linenu=arraysearch(Order,"*5928*", linenu,¶)
stoploopif linenu=0
until info("found")
stop
Order=arraychange(Order,arraychange(extract(Order,¶,linenu),"o",5,¬),linenu,¶)
...
˛/.483 recall selectorˇ˛.5928ˇ;;this works on a selection
...
until info("stopped")
˛/.5928ˇ˛.483 recall generatorˇ;stop
...
until info("stopped")
˛/.483 recall generatorˇ˛.3759 refundˇ//this works on a selection
...
loop
linenu=arraysearch(Order,"*3759*", linenu,¶)
stoploopif linenu=0
until info("found")
...
//comment this section out if order still has Backorders ;Backorder is empty
// leae these lines if refund will be issued. comment out lines in .refund if CC batch refunds will happen to avoid having to hit "enter" after every refund
if str(OrderNo) notcontains "."
«BalDue/Refund»=Paid-GrTotal
call ".refund"
Paid=Paid-«BalDue/Refund»
...
until info("stopped")
˛/.3759 refundˇ˛.recall refund w addressˇzmethod="Cash"
data= info("DatabaseName")
If «BalDue/Refund» ≥ zcheckmin OR (zcanada="Yes" AND «BalDue/Refund» > .10) OR vmail="pBm"
;	YesNo "Do you want to write a check for "+pattern(«BalDue/Refund», "$#.##")+"?"
 ;   	if clipboard()="Yes"
...
˛/spare_text_keyˇ˛FindUnprintedSouthernsˇ//testing Find Southerns with Picksheet not yet printed; it works!
	if timedifference(ztime,now())>ztimedif
Synchronize
ztime=now() ;resets sync timer
	endif
...
SelectAdditional Zip ≥ 84000 AND Zip ≤ 89999  ;UT,AZ,NM,NV
SelectAdditional Zip ≥ 90000 AND Zip ≤ 98999  ;CA,OR,HI,WA
selectwithin PickSheet = ""˛/FindUnprintedSouthernsˇ˛BatchPicksheetPrintˇ//this is to print batches of selected orders usually when we select for an item that needs to go out soon (when we hold onto orders awaiting items)
...
Also, one or more order(s) has more than 200 items. Check with Bernice or Carol about this!"
 ...
 message "Nothing to print."
 endif  ˛/BatchPicksheetPrintˇ˛copy_name_add_ph_emailˇclipboard()=Con+¶+MAd+¶+City+¶+St+¶+str(Zip)+¶+Telephone+¶+Email˛/copy_name_add_ph_emailˇ˛(canada)ˇ˛/(canada)ˇ˛SelectCanadiansˇselectall
	if timedifference(ztime,now())>ztimedif
Synchronize
ztime=now() ;resets sync timer
	endif
...
clipboard()=visiblefields()˛/field_listˇ˛Facilitation_Status_Findˇ	if folderpath(dbinfo("folder","")) CONTAINS "tally.seeds"
zlarge=0
ono=OrderNo
        	if Zip = 0 and «9SpareText» ≠ ""
        	message "This is a Canadian order. You cannot work on it here. Use the Canadian setup. This macro will stop"
...
stop ;used when Facilitation runs find avoid procedure as extra protection

˛/Facilitation_Status_Findˇ˛SourceGetˇ
